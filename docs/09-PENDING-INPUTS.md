...
>
> Last updated: **2026-09-17** — **`versionCode 11` / `0.11.0` (FRIENDS) is built and signed**
> and is the intended production release. Three things stand between it and the upload, all
> Mikhil's, in this order: **① `npx wrangler deploy` from `worker/`** (adds the static `/p`
> landing page — the app half works without it, but anyone without the app would get a 404);
...
| **FRIENDS: Worker `/p` route deployed** | 🔴 **Open — new 2026-09-17.** `npx wrangler deploy` from `worker/`, then check `https://continue-worker.gamestoplay.workers.dev/p` returns the "A FRIEND SHARED THEIR GAME PILE" page with a `Content-Security-Policy` header. | People tapping a pile link *without* the app | App Links need nothing new — verification is per host and `/.well-known/assetlinks.json` is already live — so installed apps open `/p` links regardless. Without the deploy, everyone else gets a JSON 404 instead of an install page. |
