...
| **Play App Signing SHA-256 fingerprint** | 🔴 **Open — new 2026-09-12. Blocks the friend loop.** Read it from Play Console → Test and release → **Setup** → **App integrity** → *App signing key certificate* → SHA-256, then paste it over the `PLAY_APP_SIGNING_SHA256_PENDING` placeholder in `worker/src/routes/gameLink.ts` and flip the assertion in the `PENDING:` test in `worker/test/gameLink.test.ts` from `false` to `true`. | Android App Links — whether a shared `/g/<id>` link opens the app or a browser | **Google re-signs every upload with its own key**, so the upload certificate in `app/keystore/continue-release.jks` is *not* what a Play install carries — that one is already in the file, along with the debug key, which covers sideloaded and `installDebug` builds only. ⚠️ **App Links verification fails silently**: with a wrong or placeholder fingerprint Android just opens a browser and nothing logs an error anywhere, which is why the test above exists as the reminder. **Order matters** — the Worker must be deployed with the correct fingerprint *before* an app build carrying `autoVerify` is installed, because Android verifies at install time. |
